...
local function executePlugin(pluginName, flags)
    ...
    -- Create a COMPLETE environment for the plugin
    local pluginEnv = {
        -- RootKit API Functions (direct references)
        register = function(cmdName)
            local result = Interpreter.register(cmdName)
            print("PLUGIN: Registered command: *." .. cmdName)
            return result
        end,
    ...
    -- Execute plugin with better error reporting
    local success, runtimeError = pcall(function()
        local result = pluginFunc()
        return result
    end)
    
    if not success then
        return false, "Plugin runtime error on line 5: " .. tostring(runtimeError)
    end
    
    return true, "Plugin executed successfully: " .. pluginName
end
